...
class AdaLayerNorm(nn.Module):
    ...
    使用时间嵌入 t 动态生成 LayerNorm 的尺度 (gamma) 和偏移 (beta) 参数。
    """
    def __init__(self, d_model, d_time_emb):
        super().__init__()
        # 标准 Layer Normalization
        self.norm = nn.LayerNorm(d_model)
        

        self.linear = nn.Linear(d_time_emb, 2 * d_model)

        nn.init.zeros_(self.linear.weight)
        nn.init.zeros_(self.linear.bias)

    def forward(self, x, t):
        # 1. 对输入 x 进行标准 LayerNorm
        normalized_x = self.norm(x)


        gamma_beta = self.linear(t)
        

        gamma, beta = torch.chunk(gamma_beta, 2, dim=-1)


        out = (1 + gamma.unsqueeze(1)) * normalized_x + beta.unsqueeze(1)
