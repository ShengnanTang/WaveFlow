...
def get_gp(kernel: Prior, gamma , length: int, freq: int)    :
    """
...
class Q0Dist(torch.nn.Module):
    def __init__(
        ...
        mean = torch.zeros(num_features, window_length)

        cov_batch = cov.unsqueeze(0).repeat(num_features, 1, 1)

        self.dist = MV(loc=mean.float(), covariance_matrix=cov_batch.float())
    ...
    def log_likelihood(self, x) :
        ...
        return -self.dist.log_prob(x.transpose(1, 2)) 

    def forward(self, num_samples: int) :
        # samples = self.dist.sample(torch.Size([num_samples]))

        
        cov_batch = self.cov_single.unsqueeze(0).repeat(self.num_features, 1, 1).to(torch.float64)  # [8, 60, 60]
        # 2. 均值扩展为 [num_features, window_length]
        mean_batch = torch.zeros(self.num_features, self.window_length, device=self.cov_single.device,dtype=torch.float64) 
        # 3. 构建批量单特征分布（每个特征一个 MV 分布，批量处理）
        dist = MV(loc=mean_batch, covariance_matrix=cov_batch)
        # 4. 采样：[num_samples, num_features, window_length] → 转置为 [num_samples, window_length, num_features]
        samples = dist.sample(torch.Size([num_samples])).transpose(1, 2)  # [32, 60, 8]
        return samples.to(torch.float32) 

    def gp_regression(self, x , prediction_length: int) -> MV:
        """
