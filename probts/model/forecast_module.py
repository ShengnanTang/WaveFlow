...
class ProbTSForecastModule(pl.LightningModule):
    def __init__(
        ...
        set_global_grad = False,
        **kwargs
    ):
        super().__init__()
        self.set_global_grad = set_global_grad
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        f = self.forecaster
        if getattr(f, "use_ema", False) and getattr(f, "ema_backbone", None) is not None:

            f.ema_backbone.update()

    def validation_step(self, batch, batch_idx, dataloader_idx=None):

        if self.set_global_grad:
